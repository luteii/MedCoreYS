...
{
    public partial class HastaDashboard : Form
    {
        // Modern Dashboard Renk Paleti (Hasta Paneli)
        ...
        // --- SEÇİLİ DURUMU (ACTIVE STATE) TAKİP DEĞİŞKENLERİ ---
        Button seciliAnaMenuButonu;
        Button seciliAltMenuButonu;

        public HastaDashboard()
        {
            ...
            ModernHastaPaneliniKur();
        }

        private void ModernHastaPaneliniKur()
        {
            this.Text = "MedCore YS - Hasta İşlem Paneli";
            ...
            // ==========================================
            // PANELLERİ OLUŞTUR VE Z-ORDER KİLİDİ
            // ==========================================
            ...
            // ==========================================
            // TEK PARÇA KUSURSUZ SOL PANEL ÇİZİMİ
            // ==========================================
            ...
            ikonMesajlari.SetToolTip(pnlAvatar, "Hasta Profili / Ayarlar");

            pnlAvatar.Paint += (s, e) => {
                e.Graphics.SmoothingMode = SmoothingMode.AntiAlias;
                using (SolidBrush bg = new SolidBrush(ColorTranslator.FromHtml("#10B981"))) // Zümrüt Yeşili
                    e.Graphics.FillEllipse(bg, 0, 0, 53, 53);
                TextRenderer.DrawText(e.Graphics, "HS", new Font("Segoe UI", 16, FontStyle.Bold), new Rectangle(0, 0, 54, 54), Color.White, TextFormatFlags.HorizontalCenter | TextFormatFlags.VerticalCenter);
            };
            pnlSidebar.Controls.Add(pnlAvatar);

            // --- HASTA MENÜSÜ İKON BUTONLARI ---
            IkonButonuOlustur("⌂", "Ana Sayfa", 110, (s, e) => AltMenuGuncelle("Genel Bakış", new string[] {
                "Profil Bilgilerim|ProfilBilgilerEkrani",
                "Bilgilerimi Güncelle|BilgileriGuncelleEkrani" }));

            Button btnVarsayilan = IkonButonuOlustur("◷", "Randevularım", 180, (s, e) => AltMenuGuncelle("Randevu Yönetimi", new string[] {
                "Yeni Randevu Al|YeniRandevuEkrani",
                "Geçmiş Randevularım|GecmisRandevuEkrani",
                "Randevu İptal Et|RandevuIptalEkrani" }));

            IkonButonuOlustur("✚", "Sağlık Kayıtlarım", 250, (s, e) => AltMenuGuncelle("Kayıtlar & Reçeteler", new string[] {
                "Reçetelerimi Gör|RecetelerEkrani",
                "Yatış Kayıtlarım|YatisKayitlarimEkrani",
                "Muayene Geçmişim|MuayeneGecmisimEkrani" }));
            IkonButonuOlustur("⚗", "Tahlil Sonuçları", 320, (s, e) => AltMenuGuncelle("Tahlil Sonuçları", new string[] {
                "Tahlillerimi İncele|TahlillerimEkrani",
                "Sonuç Detayları|SonucDetayEkrani" }));
            IkonButonuOlustur("💳", "Mali İşlemler", 390, (s, e) => AltMenuGuncelle("Ödemeler & Faturalar", new string[] {
                "Faturalarım|FaturalarEkrani",
                "Ödeme Geçmişi|OdemeGecmisiEkrani",
                "Online Ödeme Yap|OnlineOdemeEkrani" }));

            // İlk açılışta menü ayarları
            seciliAnaMenuButonu = btnVarsayilan;
            btnVarsayilan.Invalidate();

            // ==========================================
            // SOL ALT KÖŞEYE SABİTLENMİŞ ÇIKIŞ BUTONU
            // ==========================================
            ...
            lblSubMenuTitle.Text = "Randevu Yönetimi";
            ...
            // ==========================================
            // ÜST ÇUBUK (Top Bar - Ferahlatıldı)
            // ==========================================
            pnlTopBar.Paint += (s, e) => {
            ...
            lblKullaniciBilgi.Text = "MedCore - Hasta İşlem Paneli";
            ...
            // Varsayılan Ekran Olarak Profil / Dashboard Yükle
            AltMenuGuncelle("Genel Bakış", new string[] {
                "Profil Bilgilerim|ProfilBilgilerEkrani",
                "Bilgilerimi Güncelle|BilgileriGuncelleEkrani"
            });
            ProsedurCalistir("ProfilBilgilerEkrani", "Profil Bilgilerim");
        }

        private void AltMenuGuncelle(string baslik, string[] islemler)
        {
            lblSubMenuTitle.Text = baslik;
            seciliAltMenuButonu = null; // Kategori değiştiğinde alt menü seçimini sıfırla
            ...
            foreach (string islem in islemler)
            {
                ...
                // Cam Efekti ve Seçili Durum Kontrolü
                btnSub.Paint += (s, e) => {
                    e.Graphics.SmoothingMode = SmoothingMode.AntiAlias;
        ...
        private void ProsedurCalistir(string formAdi, string kullaniciAdi)
        {
            ...
            try
            {
                switch (formAdi)
                {
                    case "ProfilBilgilerEkrani":    uc = new Moduller_Hasta.ProfilBilgilerEkrani();    break;
                    case "BilgileriGuncelleEkrani": uc = new Moduller_Hasta.BilgileriGuncelleEkrani(); break;
                    case "YeniRandevuEkrani":        uc = new Moduller_Hasta.YeniRandevuEkrani();        break;
                    case "GecmisRandevuEkrani":      uc = new Moduller_Hasta.GecmisRandevuEkrani();      break;
                    case "RandevuIptalEkrani":       uc = new Moduller_Hasta.RandevuIptalEkrani();       break;
                    case "RecetelerEkrani":          uc = new Moduller_Hasta.RecetelerEkrani();          break;
                    case "YatisKayitlarimEkrani":    uc = new Moduller_Hasta.YatisKayitlarimEkrani();    break;
                    case "MuayeneGecmisimEkrani":    uc = new Moduller_Hasta.MuayeneGecmisimEkrani();    break;
                    case "TahlillerimEkrani":        uc = new Moduller_Hasta.TahlillerimEkrani();        break;
                    case "SonucDetayEkrani":         uc = new Moduller_Hasta.SonucDetayEkrani();         break;
                    case "FaturalarEkrani":          uc = new Moduller_Hasta.FaturalarEkrani();          break;
                    case "OdemeGecmisiEkrani":       uc = new Moduller_Hasta.OdemeGecmisiEkrani();       break;
                    case "OnlineOdemeEkrani":        uc = new Moduller_Hasta.OnlineOdemeEkrani();        break;
                    default:
        ...
        // ==========================================
        // DÜMDÜZ SOL KÖŞELİ YENİ ÇİZİM ALGORİTMASI
        // ==========================================
